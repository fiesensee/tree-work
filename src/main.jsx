  ...
  return <li className="branch">
    <article ref={card} tabIndex={isFocusRoot ? -1 : undefined} className={`task-card ${node.done ? 'completed' : ''} ${isFocusRoot ? 'focused-root' : 'can-focus'}`} onClick={event => {
      ...
      <div className="task-main">
        <button className="checkbox" role="checkbox" aria-checked={node.done} aria-label={`${node.done ? 'Reopen' : 'Complete'} ${node.title}`} disabled={busy || blocked} onClick={() => onToggle(path)} title={blocked ? 'Finish all subtasks first' : node.done ? 'Reopen task' : 'Complete task'}>
          {node.done ? <Icon name="check" size={15}/> : blocked ? <Icon name="lock" size={12}/> : null}
        </button>
        {isFocusRoot ? <span className="task-title">{node.title}</span> : <button className="task-title task-focus" title="View this task as the root" aria-label={`Focus on ${node.title}`} onClick={() => onFocus(path)}>{node.title}</button>}
        {hasChildren && <button
          className="branch-toggle"
          aria-label={`${isCollapsed ? 'Expand' : 'Collapse'} subtasks of ${node.title}`}
          aria-expanded={!isCollapsed}
          aria-controls={`subtasks-${pathKey}`}
          title={isCollapsed ? `Show ${hiddenCount} hidden tasks` : 'Collapse subtasks'}
      ...
      <div className="task-bottom">
        ...
        <button className="add-child" disabled={busy} aria-label={`Add subtask to ${node.title}`} title="Add subtask" onClick={() => onAdd(path)}><Icon name="plus" size={16}/></button>
      </div>
      {node.children.length > 0 && <div className="node-progress"><span style={{ width: `${childrenDone / node.children.length * 100}%` }}/></div>}
    </article>
    {hasChildren && <ul id={`subtasks-${pathKey}`} className="children" hidden={isCollapsed}>
...
function TaskDialog({ parent, onClose, onSubmit, busy, error }) {
  ...
  return <dialog ref={dialog} aria-labelledby="task-dialog-title" onCancel={event => { event.preventDefault(); if (!busy) onClose(); }} onClick={event => { if (event.target === event.currentTarget && !busy) onClose(); }}>
    <form onSubmit={event => { event.preventDefault(); if (title.trim() && !busy) onSubmit(title.trim()); }}>
      <div className="dialog-heading"><span className="eyebrow">{parent ? 'A SMALLER STEP' : 'A NEW BEGINNING'}</span><button type="button" className="icon-button" aria-label="Close" disabled={busy} onClick={onClose}><Icon name="close"/></button></div>
      <h2 id="task-dialog-title">{parent ? 'Add a subtask' : 'Plant a new task'}</h2>
      <p>{parent ? <>Under <strong>{parent.title}</strong></> : 'Start with the big picture. Break it down as you go.'}</p>
      <label htmlFor="task-title">Task name</label>
      <input ref={input} id="task-title" value={title} onChange={event => setTitle(event.target.value)} maxLength={500} placeholder={parent ? 'What’s the next small step?' : 'What would you like to work on?'} required disabled={busy}/>
...
function App() {
  ...
  async function save(nextTree) {
    ...
    try {
      const response = await fetch('/api/tasks', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ tree: nextTree, revision: data.revision }) });
      const value = await response.json();
      if (!response.ok) throw new Error(value.error || 'Could not save. Your file has not been changed.');
  ...
  return <main className="app" aria-label="Tree Work">
    <TreeCanvas ready={!!data} navigationKey={focusKey} navigationControls={focusedNode && <button className="text-button back-to-root" onClick={() => setFocusPath(null)}><Icon name="back" size={16}/>Back to all tasks</button>} branchControls={<>
      ...
      <span className="save-status" role="status">{busy ? 'Saving / loading…' : error ? 'File needs attention' : saved ? 'All changes saved' : 'Local workspace'}</span>
      <button className="text-button" disabled={busy} onClick={reload} title="Reload changes made in your text editor"><Icon name="refresh" size={15}/><span>Reload file</span></button>
    </>}>
      {!data ? <div className="loading-state">{error ? 'Your task file could not be opened. Fix the file and try Reload file.' : 'Opening your workspace…'}</div> : (
        <div className="forest">
          {focusedNode ? <ul className="focused-tree" aria-label={`Focused task: ${focusedNode.title}`}>
